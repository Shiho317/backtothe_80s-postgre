...
const ShowList = ({setShowList, event, user}) => {
  ...
  useEffect(() => {
    ...
    axios.post('http://localhost:8000/api/people/getlist', eventInfo)
    .then(res => {
      ...
      <div className='w-11/12 p-3 bg-white rounded-md md:w-2/3 lg:w-1/3'>
        <table className='w-full'>
          <thead>
            ...
            <th className='border bg-green-300' colSpan={1}>name</th>
            <th className='border bg-blue-500' colSpan={1}>email</th>
          </tr>
          </thead>
          <tbody>
          ...
          {peopleList && peopleList.map((list, index) => (
            <tr>
